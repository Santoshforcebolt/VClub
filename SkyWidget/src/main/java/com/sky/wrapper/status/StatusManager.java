...
public class StatusManager {
    ...
    public boolean clearStatusFlag;  //是否清除标记值
    ...
    public void show(String status, OnStatusListener listener) {
        if (currentStatusProvider != null) {
            if (!currentStatusProvider.getStatus().equals(status)) {
...
//        contentView.bringToFront();
    ...
    public boolean isHide() {
        return currentStatusProvider == null || currentStatusProvider.isHide();
    }
    ...
    public void removeAllStatus() {
        if (map.size() > 0) {
            ...
            while (iterator.hasNext()) {
                ...
                removeStatus(key);
                iterator.remove();//添加此行代码
            }
        }
    }

    /**
     * 设置此属性 在展示状态布局时 将不会记录上一次展示的状态
     *
